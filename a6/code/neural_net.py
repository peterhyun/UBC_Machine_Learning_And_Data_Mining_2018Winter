...
class NeuralNet():
    ...
    def funObj(self, weights_flat, X, y):
        ...
        activations = [X]
        for W, b in weights:
            #print(X.shape) # (50000, 784)
            #print(W.T.shape) # W = (50, 784)
            # b.shape == (50000, 50)
            Z = X @ W.T + b
            X = 1/(1+np.exp(-Z))
    ...
    def fit(self, X, y):
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(0, n, batch_size):
                weights_flat, f = findMin.SGD(self.funObj, weights_flat, X[i:i+batch_size,:], y[i:i+batch_size,:], verbose=True, alpha = alpha)
